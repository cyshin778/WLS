...
cov_factor <- c('sex', 'Marital_status_R5')
...
# # Output
# print(results_df)
write.csv(results_df, "Results/main_effect_results_250829.csv", row.names = FALSE)
...
make_interaction_plot_grouped <- function(indep_var, mod_var, data, outcome_var, COVARS) {
  ...
  quantiles_mod <- quantile(data[[mod_var]], probs = c(0.10, 0.90), na.rm = TRUE)
  group_var <- paste0(mod_var, "_group")
  
  data[[group_var]] <- cut(
    data[[mod_var]],
    breaks = c(-Inf, quantiles_mod[1], quantiles_mod[2], Inf),
  ...
  pred_min_low  <- get_extreme(pred_df, x_min, "Low (≤10th)")
  pred_min_high <- get_extreme(pred_df, x_min, "High (≥90th)")
  pred_max_low  <- get_extreme(pred_df, x_max, "Low (≤10th)")
  pred_max_high <- get_extreme(pred_df, x_max, "High (≥90th)")
  
  arrow_data <- data.frame(
    x = c(x_min, x_max),
    xend = c(x_min, x_max),
    y = c(pred_min_low$predicted, pred_max_low$predicted),
    yend = c(pred_min_high$predicted, pred_max_high$predicted),
    label = round(c(pred_min_high$predicted - pred_min_low$predicted,
                    pred_max_high$predicted - pred_max_low$predicted), 2)
  )
  
  # Plot
  p <- ggplot(pred_df, aes(x = x, y = predicted, color = group, fill = group)) +
    geom_line(size = 1.2) +
    ...
    geom_text(
      data = arrow_data,
      aes(x = x + 0.1, y = (y + yend)/2, label = label),
      color = "black",
      hjust = 0,
      inherit.aes = FALSE
    ) +
    scale_color_manual(values = c("#abdda4", "#2b83ba", "#d7191c")) +
    scale_fill_manual(values = c("#abdda4", "#2b83ba", "#d7191c")) +
    labs(
      title = paste("Moderation Effect of Neuroticism GPS"),
      x = paste("Psychological Wellbeing"),
      y = paste("Predicted late-life depressions score"),
      color = group_var,
      fill = group_var
    ) +
    coord_cartesian(xlim = c(min(pred_df$x), max(pred_df$x) + 0.4))
