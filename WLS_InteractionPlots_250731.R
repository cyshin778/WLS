...
make_curve_plot <- function(data,                       
                            indep_var,    # 그룹화할 변수 (factor 그룹)
                            mod_var,      # x축 변수 (연속, percentile)
                            outcome_var,  
                            cov_cont,                   
                            cov_factor,
                            cov_ev) {               
  ...
  # 1. Split indep_var into Low / Mid / High (10 / 80 / 10 %)
  q <- quantile(data[[indep_var]], probs = c(.10, .90), na.rm = TRUE)
  data <- data %>% 
    mutate(
      Indep_grp = cut(
        .data[[indep_var]],
  ...
  # 2. Fit linear model
  form <- reformulate(
    c(mod_var, "Indep_grp", paste0(mod_var, ":Indep_grp"), cov_cont, cov_factor, cov_ev),
    response = outcome_var
  )
  
  fit <- lm(form, data = data)
  
  # 3. Build prediction grid: 100 percentile of mod_var × 3 Indep_grp
  mod_grid <- tibble(
    mod_pct = 1:100,
    !!mod_var := quantile(data[[mod_var]], probs = seq(.01, 1, .01), na.rm = TRUE)
  )
  
  cov_means <- bind_cols(
    data %>% summarise(across(all_of(cov_cont), ~ mean(.x, na.rm = TRUE))),
    data %>% summarise(across(all_of(cov_factor), ~ mode_factor(.x))),
    data %>% summarise(across(all_of(cov_ev), ~ mean(.x, na.rm = TRUE)))
  )
  
  newdat <- tidyr::crossing(
    mod_grid,
    tibble(Indep_grp = levels(data$Indep_grp))
  ) %>% 
    bind_cols(cov_means[rep(1, nrow(.)), ])
  ...
  # 5. Compute High vs Low group difference at mod_var 10th & 90th percentile
  arrow_df <- newdat %>%
    filter(mod_pct %in% c(10, 90),
           Indep_grp %in% c("Low (≤10th)", "High (≥90th)")) %>%
    dplyr::select(mod_pct, Indep_grp, Pred) %>%
    tidyr::pivot_wider(names_from = Indep_grp,
                       values_from = Pred) %>%
    mutate(
  ...
  # 6. Plot
  ggplot(newdat,
         aes(x = mod_pct, y = Pred,
             colour = Indep_grp, fill = Indep_grp)) +
    geom_line(linewidth = 1.2) +
...
make_curve_plot <- function(data,                       
  ...
  # 2. Fit linear model
  form <- reformulate(
    c(indep_var, "Mod_grp", paste0(indep_var, ":Mod_grp"), cov_cont, cov_factor, cov_ev),
    response = outcome_var
  )
