...
for (indep_var in indep) {
  for (mod in moderators) {
    ...
    vif_list[[paste(indep_var, mod, sep = "_x_")]] <- vif_values
...
# Output
#print(results_df)
write.csv(results_df, "Results/main_effect_results_250723.csv", row.names = FALSE)
...
write.csv(final_results, "Results/moderation_results_250723.csv", row.names = FALSE)
...
target_indep <- "Depression_GPS"  # Select the independent variable manually
