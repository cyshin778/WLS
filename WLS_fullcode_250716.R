...
# Output
#print(results_df)
#write.csv(results_df, "main_effect_results_250719.csv", row.names = FALSE)
...
#write.csv(final_results, "moderation_results_250719.csv", row.names = FALSE)
...
make_interaction_plot_grouped <- function(indep_var, mod_var, data, outcome_var, covariate_str) {
  ...
  # Build formula
  formula_str <- paste(
    outcome_var, "~", group_var, "*", indep_var, "+", covariate_str, "+", paste0("EV", 1:10, collapse = " + ")
  )
...
indep_var <- "Neuroticism_GPS" # manually 
...
# 저장
write.csv(mediation_results, paste0("mediation_results_", indep_var, "_250716.csv"), row.names = FALSE)
